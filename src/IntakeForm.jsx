...
const DEMO_QUESTIONS = [
  {
    id: "q1",
    question: "Are you mainly interested in an e-commerce website, a stock and operations management system, or a combination of both?",
    type: "single",
    options: ["An e-commerce website", "A stock and operations management system", "Both", "Other"],
  },
  {
    id: "q2",
    question: "Would you prefer an independent system, or a system that works alongside your existing Hondos Center platform and software?",
    type: "single",
    options: ["An independent system", "A system that works alongside Hondos Center", "I'm not sure yet — I need advice on this", "Other"],
  },
  {
    id: "q3",
    question: "Would you like the three stores in Pyrgos, Kalamata and Amaliada to be managed together through one central system?",
    type: "single",
    options: ["Yes — all three together in one system", "No — each store managed separately", "Together — but with separate views for each store", "Other"],
  },
  {
    id: "q4",
    question: "What software are you currently using for sales, stock and product management?",
    type: "text",
    options: [],
  },
  {
    id: "q5",
    question: "Can your current software export product and stock information in Excel or CSV format?",
    type: "single",
    options: ["Yes", "I think so — but I'm not sure how", "No", "I don't know", "Other"],
  },
];
...
const UI_TEXT = {
  en: {
    ...
    subtitle: "Your answers will help us understand your needs and prepare a proposal tailored to your business.",
    ...
    success:
      "We have received your answers and will review them carefully. You will receive a detailed proposal shortly.",
    emailSubject: "Project Questionnaire",
    responseHeading: "New questionnaire response",
    nameInEmail: "Name",
    emailInEmail: "Email",
    otherPrefix: "Other",
  },
  el: {
    ...
    english: "English",
    of: "από",
    title: "Καταγραφή Αναγκών Έργου",
    subtitle:
      "Οι απαντήσεις σας θα μας βοηθήσουν να κατανοήσουμε τις ανάγκες σας και να ετοιμάσουμε μια πρόταση προσαρμοσμένη στην επιχείρησή σας.",
    fullName: "Ονοματεπώνυμο",
    email: "Email",
    question: "Ερώτηση",
    other: "Άλλο (παρακαλώ διευκρινίστε)",
    describe: "Παρακαλώ περιγράψτε...",
    typeAnswer: "Γράψτε την απάντησή σας...",
    consent:
      "Συναινώ στη συλλογή και επεξεργασία των απαντήσεών μου από την Evriel Systems αποκλειστικά για την κατανόηση των αναγκών του έργου και την προετοιμασία σχετικής πρότασης. Τα δεδομένα μου θα διαχειρίζονται σύμφωνα με τον Γενικό Κανονισμό Προστασίας Δεδομένων (GDPR), δεν θα κοινοποιούνται σε τρίτους και θα διατηρούνται μόνο για όσο απαιτείται η αξιολόγηση του έργου. Παρακαλώ μην συμπεριλάβετε εμπιστευτικές πληροφορίες πελατών ή υποθέσεων στις απαντήσεις σας.",
    submit: "Υποβολή",
    loading: "Φόρτωση ερωτηματολογίου...",
    thankYou: "Σας ευχαριστούμε για την απάντησή σας",
    success:
      "Λάβαμε τις απαντήσεις σας και θα τις εξετάσουμε προσεκτικά. Θα επικοινωνήσουμε μαζί σας σύντομα.",
    emailSubject: "Ερωτηματολόγιο Έργου",
    responseHeading: "Νέα απάντηση ερωτηματολογίου",
    nameInEmail: "Όνομα",
    emailInEmail: "Email",
    otherPrefix: "Άλλο",
  },
...
const isOtherOption = (value) => value === "Other" || value === "Άλλο";
...
export default function IntakeForm() {
  ...
  const [language, setLanguage] = useState(() => {
    ...
    return params.get("lang") === "en" ? "en" : "el";
  });
  ...
  const handleSelect = (questionId, option) => {
    if (isOtherOption(option)) {
      ...
      setAnswers((previous) => ({ ...previous, [questionId]: option }));
      setOtherText((previous) => ({ ...previous, [questionId]: "" }));
    }
  };
  ...
  const handleOtherText = (questionId, value) => {
    setOtherText((previous) => ({ ...previous, [questionId]: value }));
  };

  const getAnswer = (questionId) => {
    if (isOtherOption(answers[questionId]) && otherText[questionId]) {
      return `${t.otherPrefix}: ${otherText[questionId]}`;
    }
  ...
  const allAnswered =
    questions.every((question) => {
      ...
      if (
        ...
        (!otherText[question.id] || otherText[question.id].trim() === "")
      ) {
        return false;
      }
      return true;
    ...
    return (
      ...
      >
        <div style={{ textAlign: "center", padding: "40px 24px", maxWidth: 440 }}>
          ...
          <h2 style={{ fontSize: 22, fontWeight: 600, color: BRAND.black, margin: "0 0 10px" }}>
            {t.thankYou}
          </h2>
          <p style={{ fontSize: 15, color: BRAND.gray, margin: 0, lineHeight: 1.6 }}>
            {t.success}
          </p>
    ...
    return (
      ...
      >
        <div style={{ textAlign: "center" }}>
          <Logo />
          <p style={{ color: BRAND.gray, marginTop: 16, fontSize: 14 }}>{t.loading}</p>
        </div>
      </div>
    );
  }
      ...
      <div style={{ maxWidth: 600, margin: "0 auto", padding: "0 24px 80px" }}>
        ...
        <div style={{ marginBottom: 32 }}>
          ...
          <input
            ...
            onFocus={(event) => {
              event.target.style.borderColor = BRAND.teal;
            }}
            onBlur={(event) => {
              event.target.style.borderColor = BRAND.border;
            }}
          />
        </div>
                  ...
                  <div key={option}>
                    <div
                      ...
                      style={{
                        ...
                        marginBottom: 5,
                        borderRadius: 8,
                        border: `1.5px solid ${selected ? BRAND.teal : BRAND.border}`,
                    ...
                    >
                      <div
                        style={{
                          ...
                          height: 16,
                          borderRadius: "50%",
                          border: `2px solid ${selected ? BRAND.teal : "#d1d5db"}`,
                      ...
                      <span style={{ fontSize: 14, color: BRAND.black, lineHeight: 1.4 }}>
                        {isOther ? t.other : option}
                      </span>
                    </div>
          ...
          >
            <div
              style={{
                ...
                height: 18,
                borderRadius: 4,
                border: `2px solid ${gdprConsent ? BRAND.teal : "#d1d5db"}`,
                background: gdprConsent ? BRAND.teal : BRAND.white,
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
                flexShrink: 0,
